...
function actionChart(rows) {
  ...
  (Array.isArray(rows) ? rows : []).forEach((row) => {
    const action = classifyActivityType(row).label;
...
function eventRows(items, emptyText) {
  const values = Array.isArray(items) ? items.slice(0, 200) : [];
  if (!values.length) return `<p class="empty">${escapeHtml(emptyText)}</p>`;
  return `<div class="table-wrap"><table class="logs"><thead><tr><th>Data</th><th>Usuário</th><th>Navegador</th><th>Arquivo/URL</th></tr></thead><tbody>${values.map((item) => `<tr><td>${escapeHtml(formatDate(item.timestamp))}</td><td>${escapeHtml(item.user || '-')}</td><td>${escapeHtml(item.browser || '-')}</td><td title="${escapeHtml(item.url || '')}">${escapeHtml(item.title || item.url || '-')}</td></tr>`).join('')}</tbody></table></div>`;
}
...
function rawRows(rows) {
  ...
  return items.map((row) => `<tr><td>${escapeHtml(formatDate(row.timestamp))}</td><td>${escapeHtml(row.browser || '-')}</td><td>${escapeHtml(row.domain || (row.details && row.details.domain) || '-')}</td><td>${escapeHtml(row.typeLabel || row.displayType || classifyActivityType(row).label)}</td><td><pre>${escapeHtml(JSON.stringify(row.details || row.data || {}, null, 2))}</pre></td></tr>`).join('');
}
...
function renderReport(input) {
  ...
  const documentItems = Array.isArray(activity.documentItems) ? activity.documentItems : [];
  const downloadItems = Array.isArray(activity.downloadItems) ? activity.downloadItems : [];
...
<section class="metrics"><div class="metric"><strong>${escapeHtml(formatDuration(activity.observedMs))}</strong><span>tempo observado</span></div><div class="metric"><strong>${escapeHtml(activity.sessions || 0)}</strong><span>sessões</span></div><div class="metric"><strong>${escapeHtml(activity.events || 0)}</strong><span>eventos</span></div><div class="metric"><strong>${escapeHtml(domains.length)}</strong><span>domínios</span></div><div class="metric"><strong>${escapeHtml(activity.documents || documentItems.length || 0)}</strong><span>documentos</span></div><div class="metric"><strong>${escapeHtml(activity.downloads || downloadItems.length || 0)}</strong><span>downloads</span></div></section>
...
<details class="card collapse" open><summary>Timeline de sessões</summary><div class="collapse-body">${timelineMarkup(timeline)}</div></details>
<details class="card collapse"><summary>Documentos (${escapeHtml(documentItems.length)})</summary><div class="collapse-body">${eventRows(documentItems, 'Nenhum documento observado no período.')}</div></details>
<details class="card collapse"><summary>Downloads (${escapeHtml(downloadItems.length)})</summary><div class="collapse-body">${eventRows(downloadItems, 'Nenhum download observado no período.')}</div></details>
<details class="card collapse"><summary>Logs brutos selecionados (${escapeHtml(rows.length)})</summary><div class="collapse-body"><div class="table-wrap"><table class="logs"><thead><tr><th>Data</th><th>Navegador</th><th>Domínio</th><th>Tipo</th><th>Dados</th></tr></thead><tbody>${rawRows(rows)}</tbody></table></div></div></details>
